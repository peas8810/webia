...
# ============== STREAMLIT UI ==============
st.set_page_config(page_title="CitacaoMiner (OpenAlex/Crossref/PubMed)", layout="wide")

st.markdown("# CitacaoMiner — Streamlit\n**Minere artigos mais citados** (APIs abertas) e gere *título+resumo* sugeridos com base no seu texto.  \n"
            "> Obs.: Não faz scraping do Google Scholar; usa **OpenAlex, Crossref e PubMed** (boas práticas e termos).")

with st.sidebar:
    st.header("Configuração")
    top_n = st.number_input("Qtde por base (Top N)", 5, 200, 30, step=5)
...
if run_btn:
    try:
        ...
        st.caption("⚖️ Ética & Compliance: Não faz scraping do Google Scholar. Use este app para descobrir literatura e padronizar metadados, evitando anomalias e mesclagens indevidas.")

    except Exception as e:
        st.error(f"Erro durante a mineração: {e}")
        st.stop()
else:
    st.info("Cole o texto ou envie um .txt, ajuste as opções na barra lateral e clique em **Rodar Mineração**.")
